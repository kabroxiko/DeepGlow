...
void BusManager::setupStrip(const String &type, const String &colorOrder,
                            uint8_t pin, uint16_t count) {
  cleanupStrip();
  void *strip = nullptr;
  BusNeoPixelType ledType =
      type.equalsIgnoreCase("SK6812")
          ? BusNeoPixelType::SK6812
          : (colorOrder.equalsIgnoreCase("RGB") ? BusNeoPixelType::WS2812B_RGB
  ...
  if (ledType == BusNeoPixelType::SK6812) {
    ...
    addBus(std::unique_ptr<BusNeoPixel>(
        new BusNeoPixel(strip, count, BusNeoPixelType::SK6812)));
    ...
    addBus(std::unique_ptr<BusNeoPixel>(
        new BusNeoPixel(strip, count, BusNeoPixelType::WS2812B_RGB)));
  } else {
    ...
    addBus(std::unique_ptr<BusNeoPixel>(
        new BusNeoPixel(strip, count, BusNeoPixelType::WS2812B_GRB)));
  }
}
