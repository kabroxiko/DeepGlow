...
public:
  ...
  void broadcastOtaStatus(const String &status, const String &message = "",
                          int progress = -1);
...
private:
  ...
  void handleGetState(
      AsyncWebServerRequest *request); // convert hex->percent for user
  void handleSetState(AsyncWebServerRequest *request, uint8_t *data,
                      size_t len); // convert percent->hex from user
  void handleGetPresets(AsyncWebServerRequest *request);
  void handleSetPreset(AsyncWebServerRequest *request, uint8_t *data,
                       size_t len);
  void handleGetConfig(
      AsyncWebServerRequest *request); // convert hex->percent for user
  void handleSetConfig(AsyncWebServerRequest *request, uint8_t *data,
                       size_t len); // convert percent->hex from user
  void handleGetTimers(AsyncWebServerRequest *request);
  void handleSetTimer(AsyncWebServerRequest *request, uint8_t *data,
                      size_t len);
