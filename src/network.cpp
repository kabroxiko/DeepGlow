...
void networkSetup(Configuration &config) {
  ...
  if (config.network.ssid.length() > 0 && !apFallbackTriggered) {
    ...
    int attempts = 0;
    while (WiFi.status() != WL_CONNECTED && !apFallbackTriggered &&
    ...
    if (apFallbackTriggered || wrongPassword ||
        (millis() - wifiStart >= wifiTimeout)) {
      apFallbackTriggered = true;
    }
  }
...
void networkLoop(Configuration &config) {
  // WiFi reconnect logic: if not in AP mode and not connected, try to reconnect
  // every 10 seconds
