...
public:
    ...
    void blendTransitionFrames(const PendingTransitionState& pendingTransition, const SystemState& state, std::vector<uint32_t>& outFrame);
    ...
    struct TransitionState {
        ...
        TransitionState(uint8_t b, const std::vector<uint32_t>& c) : brightness(b), colors(c) {}
    };
